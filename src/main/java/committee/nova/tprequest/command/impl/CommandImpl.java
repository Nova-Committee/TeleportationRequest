...
public class CommandImpl {
    public static int accept(CommandContext<ServerCommandSource> ctx) {
        ...
        if (request != null) {
            ...
            src.sendFeedback(() -> Text.translatable("msg.tprequest.accepted", finalRequest.getSummary(server)).formatted(Formatting.GREEN), false);
    ...
    public static int deny(CommandContext<ServerCommandSource> ctx) {
        final ServerCommandSource src = ctx.getSource();
        final ServerPlayerEntity srcPlayer = src.getPlayer();
        if (srcPlayer == null) return 0;
        ...
        if (request != null) {
            ...
            src.sendFeedback(() -> Text.translatable("msg.tprequest.denied", finalRequest.getSummary(server)).formatted(Formatting.YELLOW), false);
            return 1;
        }
        src.sendError(Text.translatable("msg.tprequest.notfound", ""));
        return 0;
    ...
    public static int ignore(CommandContext<ServerCommandSource> ctx) {
        final ServerCommandSource src = ctx.getSource();
        final ServerPlayerEntity srcPlayer = src.getPlayer();
        if (srcPlayer == null) return 0;
        final ImmutableList<TeleportRequest> l = ServerStorage.getRequestCopied();
        ...
        final TeleportRequest finalRequest = request;
        src.sendFeedback(() -> Text.translatable("msg.tprequest.ignored", finalRequest.getSummary(server)).formatted(Formatting.YELLOW), false);
        return 1;
    }
    ...
    public static int cancel(CommandContext<ServerCommandSource> ctx) {
        ...
        if (request != null) {
            ...
            src.sendFeedback(() -> Text.translatable("msg.tprequest.cancelled", finalRequest.getSummary(server)).formatted(Formatting.YELLOW), false);
            return 1;
        }
        src.sendError(Text.translatable("msg.tprequest.notfound", ""));
        return 0;
