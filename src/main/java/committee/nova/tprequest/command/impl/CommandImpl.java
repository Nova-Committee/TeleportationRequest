...
public class CommandImpl {
    public static int accept(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!shouldShow.isEmpty()) {
            src.sendSuccess(() -> Component.translatable("msg.tprequest.following"), false);
            shouldShow.forEach(t -> src.sendSuccess(() -> t.getSummary(server).append(t.isIgnored() ?
                    Component.translatable("status.tprequest.ignored") : Component.empty()).setStyle(Style.EMPTY
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("selection.tprequest.accept.info",
        ...
        if (request != null) {
            ...
            src.sendSuccess(() -> Component.translatable("msg.tprequest.accepted", finalRequest.getSummary(server)).withStyle(ChatFormatting.GREEN), false);
    ...
    public static int deny(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!shouldShow.isEmpty()) {
            ...
            shouldShow.forEach(t -> src.sendSuccess(() -> t.getSummary(server).append(t.isIgnored() ?
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("selection.tprequest.deny.info",
                            t.getSummary(server))))), false));
            return 1;
        }
        if (request != null) {
            if (!ServerStorage.requests.remove(request)) {
                src.sendFailure(Component.translatable("msg.tprequest.already_removed", request.getSummary(server)));
                return 0;
            }
            TeleportRequest finalRequest = request;
            ...
            src.sendSuccess(() -> Component.translatable("msg.tprequest.denied", finalRequest.getSummary(server)).withStyle(ChatFormatting.YELLOW), false);
            return 1;
        }
        src.sendFailure(Component.translatable("msg.tprequest.notfound", ""));
        return 0;
    ...
    public static int ignore(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!shouldShow.isEmpty()) {
            ...
            shouldShow.forEach(t -> src.sendSuccess(() -> t.getSummary(server).setStyle(Style.EMPTY
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("selection.tprequest.ignore.info",
                            t.getSummary(server))))), false));
            return 1;
        }
        if (request == null) {
        ...
        TeleportRequest finalRequest = request;
        src.sendSuccess(() -> Component.translatable("msg.tprequest.ignored", finalRequest.getSummary(server)).withStyle(ChatFormatting.YELLOW), false);
        return 1;
    }
    ...
    public static int cancel(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (!shouldShow.isEmpty()) {
            ...
            shouldShow.forEach(t -> src.sendSuccess(() -> t.getSummary(server).setStyle(Style.EMPTY
                    ...
                    .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("selection.tprequest.cancel.info",
                            t.getSummary(server))))), false));
            return 1;
        }
        if (request != null) {
            ...
            TeleportRequest finalRequest = request;
            Utilities.getPlayer(server, request.getReceiver()).ifPresent(s -> s.displayClientMessage(Component.translatable("msg.tprequest.cancelled",
                    finalRequest.getSummary(server)).withStyle(ChatFormatting.GRAY), false));
            src.sendSuccess(() -> Component.translatable("msg.tprequest.cancelled", finalRequest.getSummary(server)).withStyle(ChatFormatting.YELLOW), false);
            return 1;
        }
        src.sendFailure(Component.translatable("msg.tprequest.notfound", ""));
        return 0;
