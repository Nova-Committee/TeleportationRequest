    ...
    public static void init(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess access, CommandManager.RegistrationEnvironment env) {
        cmds.put("trtpa", dispatcher.register(CommandManager.literal("trtpa").then(
                CommandManager.argument("player", EntityArgumentType.player())
                        .requires(p -> Utilities.checkPerm(p, PermNode.COMMON_TPA, 0)).executes(ctx -> {
                            ...
                            if (t.isCoolingDown()) {
                                src.sendError(Text.translatable("msg.tprequest.cd", Utilities.getActualSecondStr(t.getTeleportCd())));
                                return 0;
                            }
                            ...
                            src.sendFeedback(() -> Text.translatable("msg.tprequest.sent", Utilities.getActualSecondStr(timeout)).formatted(Formatting.GREEN), false);
        ...
        cmds.put("trtpaccept", dispatcher.register(CommandManager.literal("trtpaccept").then(
                CommandManager.argument("id", UuidArgumentType.uuid())
                        .requires(p -> Utilities.checkPerm(p, PermNode.COMMON_TPACCEPT, 0)).executes(ctx -> {
                            ...
                            src.sendFeedback(() -> Text.translatable("msg.tprequest.accepted",
                                    request.getSummary(server)).formatted(Formatting.GREEN), false);
        ...
        cmds.put("trtpignore", dispatcher.register(CommandManager.literal("trtpignore").then(
                CommandManager.argument("id", UuidArgumentType.uuid())
                        .requires(p -> Utilities.checkPerm(p, PermNode.COMMON_TPIGNORE, 0)).executes(ctx -> {
                            ...
                            final MinecraftServer server = src.getServer();
                            src.sendFeedback(() -> Text.translatable("msg.tprequest.ignored",
                                    request.getSummary(server)).formatted(Formatting.YELLOW), false);
                            return 1;
                        })
        ...
        cmds.put("trtplist", dispatcher.register(CommandManager.literal("trtplist")
                .executes(ctx -> {
                    ...
                    if (!sent.isEmpty()) {
                        empty = false;
                        src.sendFeedback(() -> Text.translatable("category.tprequest.sent").formatted(Formatting.AQUA), false);
                        sent.forEach(r -> {
                    ...
                    if (!received.isEmpty()) {
                        empty = false;
                        src.sendFeedback(() -> Text.translatable("category.tprequest.received").formatted(Formatting.LIGHT_PURPLE), false);
                        received.forEach(t -> {
                            ...
                            src.sendFeedback(() -> summary.styled(s -> {
                                ...
                                return s;
                                    ...
                                    Text.translatable("status.tprequest.ignored") : Text.empty()), false);
                            src.sendFeedback(() -> Text.translatable("msg.tprequest.respond.format",
                                    Text.translatable("selection.tprequest.accept").setStyle(Style.EMPTY
                                            .withColor(Formatting.GREEN)
                                            .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/trtpaccept " + id))
                                            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                    ...
                                    Text.translatable("selection.tprequest.deny").setStyle(Style.EMPTY
                                            ...
                                            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                                    Text.translatable("selection.tprequest.deny.info", summary)))),
                                    ignored ? Text.empty() : Text.translatable("selection.tprequest.ignore").setStyle(Style.EMPTY
                                            .withColor(Formatting.GRAY)
                                            .withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/trtpignore " + id))
                                            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                                    Text.translatable("selection.tprequest.ignore.info", summary))))), false);
                        });
                    }
                    if (empty) src.sendError(Text.translatable("msg.tprequest.no_pending"));
                    return 1;
                })
        ...
        dispatcher.register(CommandManager.literal("tprequest")
                .then(CommandManager.literal("help").executes(ctx -> {
                    ...
                    for (final var e : cmds.keySet()) {
                        src.sendFeedback(() -> Text.translatable("desc.tprequest." + e).formatted(Formatting.GREEN), false);
                        final List<String> alias = TeleportationRequest.getAlternativesFor(e);
                        if (!alias.isEmpty()) continue;
                        src.sendFeedback(() -> Text.translatable("desc.tprequest.alias").formatted(Formatting.YELLOW), false);
                        alias.forEach(a -> src.sendFeedback(() -> Text.literal("/" + a).formatted(Formatting.YELLOW), false));
                    }
                    return 1;
                ...
                .then(CommandManager.literal("reload").executes(ctx -> {
                    ...
                    src.sendFeedback(() -> Text.translatable("msg.tprequest.reload." + (success ? "success" : "failure")
